...
## 3.X 版本升级步骤 （最新版本3.3.6.0）
3.3.6.0版本升级时间较长，大约十五分钟左右，其他版本升级时间大约为5分钟

**请务必按照此顺序升级，后期将支持后台自动升级功能**
**升级步骤：3.2.0.0→3.2.8.0→3.2.14.0→3.2.14.1→3.3.0.0→3.3.0.4→3.3.6.0**

![](/images/upgrade/3.x最新版本升级指导图.png)

### 旗舰版堡垒机--3.X版本升级包下载

[旗舰版堡垒机3.3.0.4\_to\_3.3.6.0版本升级包](http://uhas2017.cn-gd.ufileos.com/堡垒机_upgrade_3.3.0.0_to_3.3.6.0_2019-10-14.tar)

[旗舰版堡垒机3.3.0.0\_to\_3.3.0.4版本升级包](http://uhas2017.cn-gd.ufileos.com/堡垒机_upgrade_3.3.0.0_to_3.3.0.4_2019-11-21.tar)
...
应用发布增加自定义代填节点设置，并适配华为云和AWS登录页面的密码代填

实时会话和历史会话列表增加主机地址展示，并支持搜索主机地址

快速运维的执行结果和执行日志增加主机地址展示

运维应用发布资源时，使用影子账户实现用户隔离

堡垒机增加双向认证开关

H5运维剪切板复制粘贴功能优化，仅需一次ctrl+c（字符协议时，仅需选中文本）即可完成对远端服务器文本的复制，字符协议时，仅需点击右键即可粘贴文本到远端服务器

H5全屏模式实现真正意义的全屏

列表增加条目总数、选中数量的展示

堡垒机采用Access Key ID和Access Key Secret方式提供对外的API服务
...
修复导入手动登录的应用账户时，提示密码不能为空的问题

修复对SSH资源进行改密时，会把“cat”命令替换为“cat -v”的问题

修复数据量过大，打开页面提示服务正忙的问题

修复数据库控制策略关联用户组时不生效的问题

修复在会话详情页面，运维记录展示的中文字体异常的问题

修复由于防重放导致上传文件失败问题，去除所有上传请求的防重放设计

修复创建用户时，角色展示不全的问题

修复删除部门时，未完全清理关联数据的问题

修复使用客户端运维时，登录资源失败直接退出堡垒机的问题

修复通过WinSCP下载大文件（1G左右），文件无法下载的问题

修复通过脚本调用堡垒机SFTP协议，命令不能退出的问题

修复日志删除之后，还可以通过日志备份导出的问题

修复数据库控制策略关联用户数量显示错误的问题

修复CPU突然升到100%，导致无法通过2222端口运维的问题

修复磁盘达到告警阈值，没有发送告警的问题

修复导入Web证书之后，重启堡垒机服务起不来的问题

修复运维员在主机运维模块添加标签提示资源不存在的问题

修复SSH账户的密钥无法清除的问题

修复本地播放工具频繁拖动进度条导致界面崩溃的问题

修复运维记录未记录rz/sz的问题

修复通过Web页面无法下载大文件的问题

修复H5刷屏导致浏览器标签页占用内存持续升高的问题

修复安全问题：CVE-2018-15473
